...
function preload() {
  // let assetJson;
  // Preload all the assets - Character, Prototype, Foodkit & Map

  const { loadFiles } = loadAssetsJSON();
  ...
  this.load.tilemapTiledJSON('map', '../assets/IsometricMap.json');
}

function create() {
  const map = this.make.tilemap({ key: 'map' });
  const tileset = map.addTilesetImage('character', 'Human_0_Pickup2.png');
  const platforms = map.createStaticLayer('Floor', tileset, 0, 200);
